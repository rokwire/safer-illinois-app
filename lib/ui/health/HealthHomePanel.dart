...
class _HealthHomePanelState extends State<HealthHomePanel> implements NotificationsListener {
  ...
  void initState() {
    ...
    // Produces problems if it's invoked straight on init.
    Future.delayed(Duration(seconds: 1)).then((value){
      if(Health().hasEncryptedPrivateKey && !Health().hasPrivateKey) {
        _decryptWithPassword();
      }
    });
  }
  ...
  Widget _buildStatusSection() {
    ...
    String statusName = Health().rules?.codes != null ? Health().rules?.codes[statusCode]?.displayName(rules: Health().rules) : null;
    Color statusColor = Health().rules?.codes != null ? Health().rules?.codes[statusCode]?.color ?? Styles().colors.textSurface : null;
